...
export async function recalculateSeparationReadiness(tx: Prisma.TransactionClient, ctx: RequestContext, processId: string) {
  ...
  if (nextStatus !== process.status) {
    ...
    await appendAudit(tx, ctx, {
      ...
      purpose: nextStatus === SeparationStatus.READY_TO_CLOSE ? "Exit readiness gate cleared: blocking tasks, assets and access controls are complete" : "Separation stage recalculated from governed exit controls"
    });
  ...
  return { processStatus: nextStatus, openBlockingTasks: openBlocking.length, openAssets: assetsOpen, openAccess: accessOpen };
}
