...
const COUNTRY_SET = new Set<string>(ISO_COUNTRY_CODES);

export function isIsoCountryCode(value: string): value is IsoCountryCode {
  return COUNTRY_SET.has(value.toUpperCase());
}

export function countryOptions(locale: "en" | "tr") {
  const displayNames = new Intl.DisplayNames([locale === "tr" ? "tr-TR" : "en-GB"], { type: "region" });
  return ISO_COUNTRY_CODES.map((code) => ({ code, label: displayNames.of(code) ?? code }))
    .sort((left, right) => left.label.localeCompare(right.label, locale === "tr" ? "tr-TR" : "en-GB"));
}
