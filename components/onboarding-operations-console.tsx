...
import { CheckCircle2, CircleAlert, ClipboardCheck, LockKeyhole, UserCheck } from "lucide-react";
...
export function OnboardingOperationsConsole({ tasks }: { tasks: Task[] }) {
  ...
  return <section className="onb-console card">
    ...
    <div className="onb-plan-list">{[...grouped.entries()].map(([key, planTasks]) => {
      ...
      const startRisk = planStatus !== "COMPLETED" && Number.isFinite(startAt) && startAt <= now + 72 * 60 * 60 * 1000;
