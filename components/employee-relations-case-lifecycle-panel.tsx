...
import { getEmployeeRelationsLifecycleLiveData, type EmployeeRelationsLifecycleFocus } from "@/lib/employee-relations-lifecycle-live-data";
...
export async function EmployeeRelationsCaseLifecyclePanel({ focus = {} }: { focus?: EmployeeRelationsLifecycleFocus }) {
  const [ctx, locale] = await Promise.all([getServerRequestContext(), getServerLocale()]);
  if (!ctx || !can(ctx, "cases:read")) return null;
  const data = await getEmployeeRelationsLifecycleLiveData(ctx, focus);
  ...
  const focusLabel = data.focusKind === "action"
    ? c(locale, "Corrective action focus", "Düzeltici aksiyon odağı")
    : data.focusKind === "appeal"
      ? c(locale, "Appeal review focus", "İtiraz inceleme odağı")
      : data.focusKind === "case"
        ? c(locale, "Case focus", "Vaka odağı")
        : null;
  ...
  return <div className="services-shell" style={{ marginTop: 18 }}>
    ...
    <section className="card services-panel">
      <div className="services-panel-head"><div><span className="section-kicker">{c(locale, "Governed case lifecycle", "Yönetişimli vaka yaşam döngüsü")}</span><h3>{c(locale, "Resolution & closure gates", "Çözüm ve kapanış kontrolleri")}</h3>{focusLabel ? <small>{focusLabel} · {data.focusResourceId}</small> : null}</div><span className="matrix-note">OPEN → INVESTIGATING → ACTION REQUIRED → RESOLVED → CLOSED</span></div>
      ...
      <tbody>{data.rows.length ? data.rows.map((row) => <tr key={row.id} aria-current={row.focused ? "true" : undefined}>
        <td><strong>{row.caseNumber}{row.focused ? ` · ${c(locale, "Focused", "Odak")}` : ""}</strong><small className="cell-sub">{row.caseType} · {row.title}</small></td>
        ...
        <td>{row.focused && data.focusKind === "appeal" ? c(locale, "Appeal review selected", "İtiraz incelemesi seçildi") : row.readyToClose ? <><BadgeCheck size={14}/> {c(locale, "Close ready", "Kapanışa hazır")}</> : row.readyToResolve ? c(locale, "Resolution ready", "Çözüme hazır") : row.status === "CLOSED" ? c(locale, "Terminal", "Terminal") : c(locale, "Work remains", "İş devam ediyor")}</td>
        {canWrite ? <td><EmployeeRelationsCaseLifecycleActions caseId={row.id} status={row.status} locale={locale} focusedActionId={row.focused && data.focusKind === "action" ? data.focusResourceId : undefined} actions={row.actions.map((action) => ({ id: action.id, actionType: action.actionType, status: action.status, ownerId: action.ownerId, dueAt: action.dueAt }))}/></td> : null}
