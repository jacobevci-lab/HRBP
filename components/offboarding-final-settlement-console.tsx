...
import { BadgeCheck, CircleAlert, CreditCard, LockKeyhole, ShieldCheck } from "lucide-react";
  ...
  async function transition(processId: string, action: SettlementAction) {
    ...
    try {
      const response = await fetch(`/api/offboarding/processes/${encodeURIComponent(processId)}/final-settlement`, {
        ...
        body: JSON.stringify({ action, ...(action === "PREPARE" ? { note: notes[processId]?.trim() } : {}) })
      });
  ...
  return <section className="card off-ops-console">
    <div className="off-ops-head">
      <div>
        ...
        <p>{c(
          "This workflow governs settlement evidence and separation of duties. It does not invent statutory payroll calculations: preparation, independent approval and settlement confirmation remain explicit human payroll actions.",
          "Bu akış hesap kanıtını ve görevler ayrılığını yönetir. Yasal bordro hesabı uydurmaz: hazırlama, bağımsız onay ve ödeme/tamamlama teyidi açık insan bordro aksiyonlarıdır."
        )}</p>
      </div>
