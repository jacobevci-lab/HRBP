...
function Restricted({ title, detail, returnTo, locale }: { title: string; detail: string; returnTo: string; locale: Locale }) { return <div className="card employee-restricted-card"><LockKeyhole size={22}/><div><h3>{title}</h3><p>{detail}</p><Link className="secondary-button" style={{ marginTop: 12 }} href={`/auth/sign-in?returnTo=${encodeURIComponent(returnTo)}`}>{c(locale,"Sign in with enterprise SSO","Kurumsal SSO ile giriş yap")}</Link></div></div>; }
function SearchBar({ action, query, placeholder, locale }: { action: string; query: string; placeholder: string; locale: Locale }) { return <form className="enterprise-toolbar" action={action} method="get"><div className="enterprise-search"><Search size={16}/><input name="q" defaultValue={query} placeholder={placeholder}/></div><div style={{ display: "flex", gap: 8 }}><button className="secondary-button" type="submit">{c(locale,"Search","Ara")}</button>{query ? <Link className="secondary-button" href={action}>{c(locale,"Clear","Temizle")}</Link> : null}</div></form>; }
