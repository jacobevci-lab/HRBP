...
export async function RecruitingWorkspace({ slug }: { slug:string }) {
  ...
  if (slug === "recruiting") {
    ...
    try {
      ...
      return <>
        <div className="recruit-metrics"><Metric label={c(locale,"Open requisitions","Açık talepler")} value={String(data.openRequisitions)} note={c(locale,`${data.approvalRequisitions} awaiting approval`,`${data.approvalRequisitions} onay bekliyor`)} icon={BriefcaseBusiness}/><Metric label={c(locale,"Active candidates","Aktif adaylar")} value={String(data.activeCandidates)} note={c(locale,"Across active pipeline stages","Aktif aday aşamalarında")} icon={UsersRound}/><Metric label={c(locale,"Interview pipeline","Mülakat havuzu")} value={String(data.interviewPipeline)} note={c(locale,"Interview + assessment","Mülakat + değerlendirme")} icon={CalendarClock}/><Metric label={c(locale,"Offers","Teklifler")} value={String(data.activeOffers)} note={c(locale,`${data.awaitingSignature} awaiting response`,`${data.awaitingSignature} yanıt bekliyor`)} icon={FileCheck2}/></div>
        ...
        <div className="recruit-grid"><section className="card pipeline-card"><div className="recruit-title"><div><h3>{c(locale,"Candidate pipeline","Aday havuzu")}</h3><p>{c(locale,"Restricted candidate records linked to approved requisitions.","Onaylı işe alım taleplerine bağlı kısıtlı aday kayıtları.")}</p></div><span>{c(locale,"Live PostgreSQL","Canlı PostgreSQL")}</span></div><div className="pipeline-board">{data.pipeline.map((column)=><div className="pipeline-column" key={column.rawStage}><header><strong>{localStatus(locale,column.rawStage) || column.stage}</strong><span>{column.count}</span></header>{column.people.length ? column.people.map((person)=><article key={person.id}><div className="candidate-avatar">{person.name.split(" ").map(n=>n[0]).slice(0,2).join("")}</div><div><strong>{person.name}</strong><small>{person.requisition}</small></div><em>{person.appliedAt}</em></article>) : <article><div><strong>{c(locale,"No records","Kayıt yok")}</strong><small>{c(locale,"Stage is currently empty","Bu aşama şu anda boş")}</small></div></article>}<button disabled>{column.count} {c(locale,"total records","toplam kayıt")}</button></div>)}</div></section>
        <section className="card requisition-card"><div className="recruit-title"><div><h3>{c(locale,"Requisitions","İşe alım talepleri")}</h3><p>{c(locale,"Live position-backed hiring demand","Canlı pozisyon destekli işe alım ihtiyacı")}</p></div><button className="secondary-button" disabled>{c(locale,"Approval queue","Onay kuyruğu")}</button></div><div className="table-wrap"><table className="enterprise-table"><thead><tr><th>{c(locale,"Requisition","Talep")}</th><th>{c(locale,"Org / location","Org / lokasyon")}</th><th>{c(locale,"Hiring manager","İşe alım yöneticisi")}</th><th>{c(locale,"Recruiter","İşe alım uzmanı")}</th><th>{c(locale,"Candidates","Adaylar")}</th><th>{c(locale,"Status","Durum")}</th><th>{c(locale,"Target","Hedef")}</th></tr></thead><tbody>{data.requisitions.length ? data.requisitions.map(r=><tr key={r.id}><td><strong className="cell-strong">{r.title}</strong><small className="cell-sub">{r.id}</small></td><td><strong className="cell-strong">{r.org}</strong><small className="cell-sub">{r.location}</small></td><td>{r.hiringManager}</td><td>{r.recruiter}</td><td>{r.candidates}</td><td><em className={`pill ${r.status.toLowerCase().replaceAll(" ", "-")}`}>{localStatus(locale,r.status)}</em></td><td>{r.target}</td></tr>) : <tr><td colSpan={7} style={{ textAlign:"center", padding:28 }}>{c(locale,"No requisitions created yet.","Henüz işe alım talebi oluşturulmadı.")}</td></tr>}</tbody></table></div></section></div>
        <div className="privacy-strip"><ShieldCheck size={17}/><div><strong>{c(locale,"Recruiting privacy boundary","İşe alım gizlilik sınırı")}</strong><p>{c(locale,"Candidate records remain Restricted and only become employee history through a controlled Hire transition.","Aday kayıtları Kısıtlı kalır ve yalnızca kontrollü İşe Al geçişiyle çalışan geçmişine dönüşür.")}</p></div><span>{c(locale,"Policy enforced","Politika uygulanıyor")}</span></div>
      </>;
    } catch (error) {
  ...
  if (slug === "onboarding") {
    ...
    try {
      ...
      return <>
        <div className="recruit-metrics"><Metric label={c(locale,"Active journeys","Aktif yolculuklar")} value={String(data.preboarding)} note={c(locale,"Open onboarding plans","Açık işe başlatma planları")} icon={UserPlus}/><Metric label={c(locale,"Tasks complete","Tamamlanan görevler")} value={`${data.taskCompletion}%`} note={c(locale,"Across active plans","Aktif planlar genelinde")} icon={BadgeCheck}/><Metric label={c(locale,"Blockers","Engeller")} value={String(data.blockers)} note={c(locale,"Blocked onboarding tasks","Engelli işe başlatma görevleri")} icon={CircleAlert}/><Metric label={c(locale,"Start readiness","Başlangıç hazırlığı")} value={`${data.readiness}%`} note={c(locale,"Average journey readiness","Ortalama yolculuk hazırlığı")} icon={ShieldCheck}/></div>
        {operations ? <OnboardingOperationsConsole tasks={operations}/> : null}
        <div className="onboarding-grid"><section className="card"><div className="recruit-title"><div><h3>{c(locale,"Onboarding journeys","İşe başlatma yolculukları")}</h3><p>{c(locale,"Live hire-to-day-one orchestration across People, IT and managers.","Çalışanlar, IT ve yöneticiler arasında işe alımdan ilk güne canlı orkestrasyon.")}</p></div><button className="secondary-button" disabled>{c(locale,"Journey templates","Yolculuk şablonları")}</button></div><div className="journey-list">{data.journeys.length ? data.journeys.map(p=><article key={p.id}><div className="journey-avatar">{p.initials}</div><div className="journey-person"><strong>{p.name}</strong><small>{p.role} · {c(locale,"Starts","Başlangıç")} {p.start}</small></div><div className="journey-owner"><small>{c(locale,"Owner","Sorumlu")}</small><strong>{p.owner}</strong></div><div className="journey-progress"><span><i style={{width:`${p.progress}%`}}/></span><small>{p.progress}% {c(locale,"ready","hazır")}</small></div><em className={p.blockers ? "journey-blocked" : "journey-healthy"}>{p.blockers ? `${p.blockers} ${c(locale,p.blockers>1?"blockers":"blocker","engel")}` : localStatus(locale,p.status)}</em></article>) : <div style={{ padding:24 }}>{c(locale,"No active onboarding journeys.","Aktif işe başlatma yolculuğu yok.")}</div>}</div></section><section className="card"><div className="recruit-title"><div><h3>{c(locale,"Cross-functional controls","Fonksiyonlar arası kontroller")}</h3><p>{c(locale,"Grouped from live onboarding tasks","Canlı işe başlatma görevlerinden gruplandı")}</p></div></div><div className="task-control-list">{data.controls.length ? data.controls.map(t=><div key={t.title}><span className={t.risk === "Healthy" ? "task-ok" : "task-watch"}/><div><strong>{t.title}</strong><small>{t.owner} · {c(locale,"Due","Son tarih")} {t.due}</small></div><b>{t.completion}</b></div>) : <div style={{ padding:20 }}>{c(locale,"No task controls configured.","Görev kontrolü yapılandırılmadı.")}</div>}</div><div className="onboarding-policy"><ShieldCheck size={17}/><p>{c(locale,"Sensitive identity documents remain in the restricted vault. IT receives provisioning attributes, not identity-document contents.","Hassas kimlik dokümanları kısıtlı kasada kalır. IT kimlik dokümanı içeriğini değil, provisioning özniteliklerini alır.")}</p></div></section></div>
      </>;
    } catch (error) {
