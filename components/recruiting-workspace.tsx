...
function OperationsUnavailableNotice({ domain, locale }: { domain: string; locale: Locale }) {
  return <div className="ats-notice error" style={{ marginBottom: 14 }}><span><CircleAlert size={15}/></span><div><strong>{c(locale,`${domain} write console is temporarily unavailable`,`${domain} yazma konsolu geçici olarak kullanılamıyor`)}</strong><small style={{ display:"block", marginTop:4 }}>{c(locale,"Live read data remains available. Protected write controls are withheld until the operational data plane recovers.","Canlı okuma verileri kullanılabilir durumda. Operasyonel veri katmanı düzelene kadar korumalı yazma kontrolleri gösterilmiyor.")}</small></div></div>;
}
...
export async function RecruitingWorkspace({ slug }: { slug:string }) {
  ...
  if (slug === "recruiting") {
    ...
    let data: Awaited<ReturnType<typeof getRecruitingWorkspaceData>>;
    try {
      data = await getRecruitingWorkspaceData(ctx);
    } catch (error) {
    ...
    let operations: Awaited<ReturnType<typeof getRecruitingOperationsData>> | null = null;
    let operationsUnavailable = false;
    if (can(ctx, "recruiting:write")) {
      try {
        operations = await getRecruitingOperationsData(ctx.tenantId);
      } catch (error) {
        console.error("[HRBP] Recruiting operations data failed; live read view remains available", error);
    ...
    return <>
      ...
      {operationsUnavailable ? <OperationsUnavailableNotice domain={c(locale,"Recruiting","İşe Alım")} locale={locale}/> : null}
      {operations ? <RecruitingOperationsConsole positions={operations.positions} users={operations.users} requisitions={operations.requisitions} applications={operations.applications} canApprove={can(ctx, "recruiting:approve")}/> : null}
  ...
  if (slug === "onboarding") {
    ...
    let data: Awaited<ReturnType<typeof getOnboardingWorkspaceData>>;
    try {
      data = await getOnboardingWorkspaceData(ctx);
    } catch (error) {
    ...
    let operations: Awaited<ReturnType<typeof getOnboardingOperationsData>> | null = null;
    let operationsUnavailable = false;
    if (can(ctx, "onboarding:write")) {
      try {
        operations = await getOnboardingOperationsData(ctx);
      } catch (error) {
        console.error("[HRBP] Onboarding operations data failed; live read view remains available", error);
        operationsUnavailable = true;
      }
    }
    ...
    return <>
      ...
      {operationsUnavailable ? <OperationsUnavailableNotice domain={c(locale,"Onboarding","İşe Başlatma")} locale={locale}/> : null}
