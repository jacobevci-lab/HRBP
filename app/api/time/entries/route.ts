...
export async function GET(request: Request) {
  ...
  const data = await db.timeEntry.findMany({
    where: { tenantId: ctx.tenantId, ...(from || to ? { workDate: { ...(from ? { gte: new Date(from) } : {}), ...(to ? { lte: new Date(to) } : {}) } } : {}) },
...
export async function POST(request: Request) {
  ...
  const data = await db.$transaction(async (tx) => {
    ...
    const entry = await tx.timeEntry.create({ data: { tenantId: ctx.tenantId, employmentId: body.employmentId!, workDate: new Date(body.workDate!), startAt: body.startAt ? new Date(body.startAt) : undefined, endAt: body.endAt ? new Date(body.endAt) : undefined, minutes: body.minutes!, overtimeMinutes: body.overtimeMinutes ?? 0, source: body.source, status: body.status ?? TimeEntryStatus.DRAFT } });
