...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.active !== undefined && typeof body.active !== "boolean") return Response.json({ error: "active must be boolean." }, { status: 400 });
  if (body.active === undefined && body.name === undefined && body.defaultSlaMinutes === undefined) return Response.json({ error: "At least one queue field is required." }, { status: 400 });

  const result = await db.$transaction(async (tx) => {
    const queue = await tx.hRServiceQueue.findFirst({ where: { id, tenantId: ctx.tenantId } });
    if (!queue) throw new Error("NOT_FOUND");
    if (body.active === false && queue.active) {
    ...
    const updated = await tx.hRServiceQueue.update({ where: { id: queue.id }, data: { ...(name !== undefined ? { name } : {}), ...(body.defaultSlaMinutes !== undefined ? { defaultSlaMinutes: sla ?? null } : {}), ...(body.active !== undefined ? { active: body.active } : {}) } });
