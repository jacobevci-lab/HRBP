  ...
  try {
    const data = await db.$transaction(async (tx) => {
      const enrollment = await tx.benefitEnrollment.findFirst({
        ...
        select: { id: true, employmentId: true, status: true, effectiveFrom: true, effectiveTo: true }
      });
    ...
    if (code === "INVALID_TRANSITION") return Response.json({ error: "Benefit enrollment transition is not allowed from the current state." }, { status: 409 });
