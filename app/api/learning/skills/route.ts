...
export async function GET(request: Request) {
  ...
  const data = await db.skill.findMany({
    ...
    include: { _count: { select: { employments: { where: { ...employmentIdFilter(scope) } } } } }
  });
...
export async function POST(request: Request) {
  ...
  if (employmentId || skillId) {
    ...
    if (data === "NOT_FOUND") return Response.json({ error: "Employment or skill not found in tenant." }, { status: 404 });
