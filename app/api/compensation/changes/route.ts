...
export async function GET(request: Request) {
  ...
  const data = await db.compensationChange.findMany({ where: { tenantId: ctx.tenantId }, orderBy: [{ effectiveAt: "desc" }, { createdAt: "desc" }], include: { employment: { select: { id: true, person: { select: { employeeNumber: true, givenName: true, familyName: true } }, position: { select: { title: true, grade: true } } } } } });
