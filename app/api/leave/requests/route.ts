...
export async function GET(request: Request) {
  ...
  const data = await db.leaveRequest.findMany({ where: { tenantId: ctx.tenantId }, orderBy: [{ startsAt: "asc" }, { createdAt: "desc" }], include: { leaveType: true, employment: { select: { id: true, person: { select: { employeeNumber: true, givenName: true, familyName: true } }, position: { select: { title: true } } } } } });
...
export async function POST(request: Request) {
  ...
  const data = await db.$transaction(async (tx) => {
    ...
    const leave = await tx.leaveRequest.create({ data: { tenantId: ctx.tenantId, employmentId: body.employmentId!, leaveTypeId: body.leaveTypeId!, startsAt: new Date(body.startsAt!), endsAt: new Date(body.endsAt!), units: body.units!, reason: body.reason, status: LeaveRequestStatus.PENDING } });
