...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await db.$transaction(async (tx) => {
    ...
    return updated;
  }).catch((error) => error instanceof Error ? error.message : Promise.reject(error));

  if (result === "NOT_FOUND") return Response.json({ error: "Review cycle not found in tenant." }, { status: 404 });
  if (result === "INVALID_TRANSITION") return Response.json({ error: "Cycle transition is not allowed from the current state." }, { status: 409 });
  if (result === "INVALID_DATES") return Response.json({ error: "Cycle end date must be after its start date." }, { status: 400 });
  if (typeof result === "string" && result.startsWith("UNFINISHED:")) return Response.json({ error: `${result.split(":")[1]} reviews must be finalized before the cycle can be finalized.` }, { status: 409 });
