...
export async function GET(request: Request) {
  ...
  const data = await db.performanceReview.findMany({ where: { tenantId: ctx.tenantId }, orderBy: { updatedAt: "desc" }, include: { cycle: { select: { id: true, name: true, status: true } } }, take: 300 });
...
export async function POST(request: Request) {
  ...
  if (!data) return Response.json({ error: "Cycle or employment record not found in tenant." }, { status: 404 });
