...
import { recalculateSeparationReadiness, terminalExitTaskStatuses } from "@/lib/offboarding-readiness";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; taskId: string }> }) {
  ...
  try {
    ...
    if (code === "PROCESS_NOT_FOUND") return Response.json({ error: "Separation process disappeared during readiness recalculation." }, { status: 409 });
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2034") return Response.json({ error: "Exit task changed concurrently. Refresh and try again." }, { status: 409 });
