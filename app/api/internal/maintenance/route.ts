...
export async function POST(request: Request) {
  ...
  // Audited lifecycle normalizers and readiness escalation run serially so
  ...
  const offboardingReadiness = await queueOffboardingReadinessReminders();
  ...
  return Response.json({ data: { ...data, benefitsLifecycle, learningLifecycle, recruitingLifecycle, onboardingReadiness, offboardingReadiness, workflowReminders, learningReminders, successionReminders, developmentPlanReminders, auditIntegrity } });
}
